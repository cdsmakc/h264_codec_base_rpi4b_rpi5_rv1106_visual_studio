...
--------------------------------------------------------------------------------
    ...
    Module name  : Camera
    Date created : 2025年4月30日   14时38分46秒
    Author       : 
    Description  : 
*******************************************************************************/
...
/*******************************************************************************
- Function    : __RCE_CAM_Close264File
- Description : 本函数打开输出文件。
- Input       : VOID
- Output      : NULL
- Return      : INT
...
INT __RCE_CAM_MapBuffers(VOID)
{
    ...
    for(iLoopBuf = 0 ; iLoopBuf < RCE_CAM_CAMERA_ALLOCATE_BUFFER_NUM ; iLoopBuf++)
    {
        ...
        g_auiBufferSize[iLoopBuf] = stBuf.length ;
    ...
    for(iLoopBuf = 0 ; iLoopBuf < RCE_CAM_CAMERA_ALLOCATE_BUFFER_NUM ; iLoopBuf++)
    {
        printf("-- Frame buffer :%d   address :0x%016llX    size:%d\n", 
               iLoopBuf, 
               (UINT64)g_apvBufferAddr[iLoopBuf], 
...
{
    enum v4l2_buf_type enType = V4L2_BUF_TYPE_VIDEO_CAPTURE ;
    INT                iRetVal ;
